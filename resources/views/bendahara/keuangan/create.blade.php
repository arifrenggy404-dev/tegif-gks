...
<div class="max-w-xl mx-auto px-2 py-2 sm:py-4">
    <div class="mb-6 sm:mb-8 flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
        <div class="order-2 sm:order-1">
            ...
            <p class="text-stone-500 text-xs sm:text-sm mt-1 pl-3 sm:pl-4">Pencatatan arus kas masuk dan keluar kas jemaat</p>
        </div>
    ...
    <div class="bg-white rounded-2xl border border-stone-200 p-5 sm:p-6 md:p-8 shadow-sm">
        <form action="{{ route('bendahara.keuangan.store') }}" method="POST" class="space-y-5 sm:space-y-6">
            @csrf
            ...
            <div>
                ...
                <div class="relative rounded-xl shadow-sm">
                    ...
                    <input type="number" name="total" id="input-total" value="{{ old('total') }}"
            ...
            <div>
                <label class="block text-xs sm:text-sm font-bold text-stone-700 mb-1.5">
                    Keterangan
                    <span class="text-stone-400 font-normal text-[11px] sm:text-xs ml-1">(opsional)</span>
                </label>
                <textarea name="keterangan" rows="3"
                          placeholder="Contoh: Persembahan minggu ke-1, Pembelian ATK, dll."
                          class="w-full px-3.5 py-2.5 bg-white border border-stone-200 rounded-xl text-stone-800 placeholder-stone-300 focus:outline-none focus:border-[#143222] focus:ring-4 focus:ring-emerald-50/50 transition-all text-xs sm:text-sm shadow-sm resize-none leading-relaxed">{{ old('keterangan') }}</textarea>
            </div>
...
<script>
    document.getElementById('input-total').addEventListener('keydown', function(e) {
        // Blokir tombol 'e', 'E', '+', '-', '.', dan ',' demi keamanan data kas
