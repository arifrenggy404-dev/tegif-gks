...
<div class="max-w-3xl mx-auto pb-10">
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
        <div>
            ...
            <h2 class="text-3xl font-extrabold text-slate-800 tracking-tight">Edit Jadwal Ibadah</h2>
            <p class="text-sm text-slate-500 font-medium mt-1">Perbarui rincian pelayanan untuk ibadah minggu.</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200/60 overflow-hidden shadow-sm relative">
        ...
        <form action="{{ route('admin.jadwal-ibadah.update', $jadwalIbadah) }}" method="POST" id="formEditJadwal" onsubmit="return handleUpdate(event, this)" class="p-6 md:p-10 space-y-8">
            @csrf
            @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select name="id_pelayan" required class="input-modern w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-800 focus:outline-none cursor-pointer">
                            ...
                            @foreach($pelayan as $p)
                                <option value="{{ $p->id_pelayan }}" {{ old('id_pelayan', $jadwalIbadah->id_pelayan) == $p->id_pelayan ? 'selected' : '' }}>{{ $p->nama_pelayan }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="id_pendamping" required class="input-modern w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-800 focus:outline-none cursor-pointer">
                            ...
                            @foreach($pelayan as $p)
                                <option value="{{ $p->id_pelayan }}" {{ old('id_pendamping', $jadwalIbadah->id_pendamping) == $p->id_pelayan ? 'selected' : '' }}>{{ $p->nama_pelayan }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
            ...
            <div>
                ...
                <div class="space-y-5">
                    <div>
                        ...
                        <input type="text" name="tema" value="{{ old('tema', $jadwalIbadah->tema) }}" placeholder="Contoh: Kasih yang Tak Bersyarat"
                    ...
                    <div>
                        ...
                        <input type="text" name="ayat_bacaan" value="{{ old('ayat_bacaan', $jadwalIbadah->ayat_bacaan) }}" required
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="nats_pembimbing" value="{{ old('nats_pembimbing', $jadwalIbadah->nats_pembimbing) }}"
                                class="input-modern w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-800 focus:outline-none">
                        </div>
                        <div>
                            <label class="block text-sm font-bold text-slate-700 mb-2">Ayat Firman</label>
                            <input type="text" name="ayat_firman" value="{{ old('ayat_firman', $jadwalIbadah->ayat_firman) }}"
                                class="input-modern w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-800 focus:outline-none">
                        </div>
                    </div>
                </div>
            ...
            <div>
                ...
                <div class="space-y-5">
                    <div>
                        ...
                        <textarea name="berita_anugerah" rows="3"
                            class="input-modern w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-800 focus:outline-none">{{ old('berita_anugerah', $jadwalIbadah->berita_anugerah) }}</textarea>
                    </div>
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-2">Petunjuk Hidup Baru</label>
                        <textarea name="petunjuk_hidup_baru" rows="3"
                            class="input-modern w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-800 focus:outline-none">{{ old('petunjuk_hidup_baru', $jadwalIbadah->petunjuk_hidup_baru) }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="datetime-local" name="waktu" value="{{ old('waktu', $jadwalIbadah->waktu->format('Y-m-d\TH:i')) }}" required
                            class="input-modern w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-800 focus:outline-none">
                    </div>
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-2">Lokasi Ibadah <span class="text-red-500">*</span></label>
                        <input type="text" name="lokasi" value="{{ old('lokasi', $jadwalIbadah->lokasi) }}" required
                            class="input-modern w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-800 focus:outline-none">
                    </div>
                </div>
            </div>
