...
<div class="max-w-3xl mx-auto pb-10">
    ...
    <div class="mb-8">
        ...
        <h2 class="text-3xl font-extrabold text-slate-800 mt-2 tracking-tight">Edit Warta Mimbar</h2>
        <p class="text-slate-500 text-sm mt-1">Perbarui rincian pengumuman warta mimbar.</p>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200/60 shadow-sm overflow-hidden relative">
        ...
        <form action="{{ route('admin.wartamimbar.update', $wartamimbar) }}" method="POST" id="formEditWarta" onsubmit="return handleUpdate(event, this)" class="p-6 md:p-10 space-y-6">
            @csrf 
            @method('PUT')
            ...
            <div>
                ...
                <input type="date" name="tanggal" value="{{ old('tanggal', $wartamimbar->tanggal->format('Y-m-d')) }}" required 
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="id_ibadah" required class="input-modern w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-800 focus:outline-none cursor-pointer">
                        ...
                        @foreach($ibadah as $ib)
                            <option value="{{ $ib->id_ibadah }}" {{ old('id_ibadah', $wartamimbar->id_ibadah) == $ib->id_ibadah ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select name="id_pelayanan_pa" required class="input-modern w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-800 focus:outline-none cursor-pointer">
                        ...
                        @foreach($pa as $p)
                            <option value="{{ $p->id_pelayanan_pa }}" {{ old('id_pelayanan_pa', $wartamimbar->id_pelayanan_pa) == $p->id_pelayanan_pa ? 'selected' : '' }}>
            ...
            <div>
                ...
                <textarea name="informasi" rows="6" required 
                          class="input-modern w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-800 focus:outline-none">{{ old('informasi', $wartamimbar->informasi) }}</textarea>
            </div>
...
<script>
    function handleUpdate(event, form) {
        ...
        Swal.fire({
            title: 'Memperbarui...',
            text: 'Data warta sedang disimpan ke sistem.',
            icon: 'info',
