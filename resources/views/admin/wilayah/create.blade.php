...
<div class="max-w-3xl mx-auto pb-10">
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
        <div>
            ...
            <h2 class="text-3xl font-extrabold text-slate-800 tracking-tight">Tambah Wilayah</h2>
            <p class="text-sm text-slate-500 font-medium mt-1">Buat data wilayah pelayanan jemaat baru.</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200/60 overflow-hidden shadow-sm relative">
        ...
        <form action="{{ route('admin.wilayah.store') }}" method="POST" id="formTambahWilayah" onsubmit="return handleSimpan(event, this)" class="p-6 md:p-10 space-y-6">
            @csrf

            <div>
                <label class="block text-sm font-bold text-slate-700 mb-2">Nama Wilayah <span class="text-red-500">*</span></label>
                <input type="text" name="nama_wilayah" value="{{ old('nama_wilayah') }}" required
                    placeholder="Contoh: Wilayah Kanatang Barat"
...
<script>
    function handleSimpan(event, form) {
        ...
        Swal.fire({
            title: 'Menyimpan...',
            text: 'Data wilayah sedang diproses.',
            icon: 'info',
