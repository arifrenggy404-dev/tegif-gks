...
<div class="max-w-3xl mx-auto pb-10">
    ...
    <div class="mb-8">
        ...
        <h2 class="text-3xl font-extrabold text-slate-800 mt-2 tracking-tight">Edit Asset</h2>
        <p class="text-slate-500 text-sm mt-1">Perbarui informasi data aset gereja.</p>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200/60 shadow-sm overflow-hidden relative">
        ...
        <form action="{{ route('admin.asset.update', $asset) }}" method="POST" id="formEditAsset" onsubmit="return handleUpdate(event, this)" class="p-6 md:p-10 space-y-6">
            @csrf 
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="number" name="jumlah" value="{{ old('jumlah', $asset->jumlah) }}" min="1" required
                           class="input-modern w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-800 focus:outline-none">
                </div>
                ...
                <div>
                    ...
                    <select name="kondisi" required class="input-modern w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-800 focus:outline-none cursor-pointer">
                        @foreach(['layak'=>'Layak', 'kurang_layak'=>'Kurang Layak', 'tidak_layak'=>'Tidak Layak'] as $val => $label)
                            <option value="{{ $val }}" {{ old('kondisi', $asset->kondisi) == $val ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
...
<script>
    function handleUpdate(event, form) {
        ...
        Swal.fire({
            title: 'Memperbarui...',
            text: 'Data aset sedang disimpan.',
            icon: 'info',
