...
<div class="max-w-full overflow-x-hidden px-1 py-2">
    ...
    <div class="bg-white p-4 rounded-2xl border border-stone-200 mb-5 shadow-sm">
        <form action="{{ route('pengurus.wilayah.index') }}" method="GET" class="flex flex-col sm:flex-row gap-3">
            ...
            <div class="flex gap-2">
                ...
                @if(request('search'))
                    <a href="{{ route('pengurus.wilayah.index') }}" class="flex-1 sm:flex-none bg-stone-100 text-stone-600 px-4 py-2.5 rounded-xl text-sm font-bold hover:bg-stone-200 transition border border-stone-300 flex items-center justify-center shadow-sm">
                        Reset
                    </a>
                @endif
            </div>
